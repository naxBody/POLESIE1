...
-- Хранит прогноз спроса на основе истории заказов
-- ============================================
...
-- Основные производственные планы с расчетами
-- ============================================
...
-- Расчет материалов для каждого плана производства
-- ============================================
...
-- Почасовое/посменное планирование работ
-- ============================================
...
-- Детальный расчет себестоимости по статьям затрат
-- ============================================
...
SELECT 
    ...
    pmr.is_critical,
    CASE 
        WHEN pmr.quantity_available < pmr.quantity_required THEN 'DEFCIT'
        ELSE 'OK'
...
SELECT 
    ...
    -- Средняя эффективность по графикам
    ...
    -- Количество дефицитных материалов
...
-- Добавляем анализ спроса (если есть продукция)
...
('PLAN-2025-001', 1, 100.000, 45.000, 'in_progress', '2025-01-15', '2025-01-31', 1, 125000.00, 35000.00, 18000.00, 178000.00, 1780.00, 1, 'План производства трансформаторов ТМГ-10'),
('PLAN-2025-002', 2, 50.000, 0.000, 'approved', '2025-01-20', '2025-02-10', 3, 85000.00, 22000.00, 12000.00, 119000.00, 2380.00, 1, 'План производства КСО-393'),
('PLAN-2025-003', 3, 150.000, 0.000, 'draft', '2025-02-01', '2025-02-28', 4, 45000.00, 15000.00, 8000.00, 68000.00, 453.33, 1, 'План производства щитов ЩРн');
...
INSERT INTO `production_material_requirements` (`plan_id`, `material_id`, `norm_per_unit`, `quantity_required`, `quantity_available`, `quantity_to_purchase`, `estimated_cost`, `supplier_id`, `is_critical`, `notes`) VALUES
(2, 4, 1.50000, 75.000, 40.000, 35.000, 42000.00, 4, TRUE, 'Сталь для корпуса КСО'),
(2, 6, 2.00000, 100.000, 100.000, 0.000, 20000.00, 5, FALSE, 'Комплектующие электрические'),
(2, 7, 0.25000, 12.500, 5.000, 7.500, 15000.00, 6, TRUE, 'Кабель силовой'),
...
(1, 'material', 'Медь обмоточная', 15.000, 5000.0000, 75000.00, 'кг', 42.13, 'Основной материал'),
(1, 'material', 'Алюминий', 8.000, 2250.0000, 18000.00, 'кг', 10.11, 'Конструкционный материал'),
(1, 'material', 'Масло трансформаторное', 2500.000, 14.0000, 35000.00, 'л', 19.66, 'Изоляционная среда'),
(1, 'material', 'Сталь электротехническая', 100.000, 250.0000, 25000.00, 'кг', 14.04, 'Магнитопровод'),
(1, 'material', 'Изоляционные материалы', 50.000, 290.0000, 14500.00, 'кг', 8.15, 'Изоляция'),
(1, 'labor', 'Основные рабочие', 800.000, 35.0000, 28000.00, 'час', 15.73, 'ФОТ основных рабочих'),
(1, 'labor', 'Вспомогательный персонал', 200.000, 25.0000, 5000.00, 'час', 2.81, 'Подсобные работы'),
(1, 'labor', 'Инженерно-технические работники', 100.000, 20.0000, 2000.00, 'час', 1.12, 'ИТР'),
(1, 'overhead', 'Аренда помещений', 1.000, 8000.0000, 8000.00, 'месяц', 4.49, 'Производственные помещения'),
(1, 'overhead', 'Амортизация оборудования', 1.000, 6000.0000, 6000.00, 'месяц', 3.37, 'Станки и линии'),
(1, 'overhead', 'Коммунальные услуги', 1.000, 4000.0000, 4000.00, 'месяц', 2.25, 'Электроэнергия, вода'),
(1, 'energy', 'Электроэнергия технологическая', 5000.000, 0.3500, 1750.00, 'кВт*ч', 0.98, 'Технологические нужды'),
(1, 'other', 'Упаковка и маркировка', 100.000, 25.0000, 2500.00, 'шт', 1.40, 'Тара и упаковка');

-- Добавляем калькуляцию для PLAN-2025-002
INSERT INTO `production_costing` (`plan_id`, `cost_type`, `cost_item`, `amount`, `unit_cost`, `total_cost`, `allocation_base`, `percentage`, `notes`) VALUES
(2, 'material', 'Сталь конструкционная', 75.000, 320.0000, 24000.00, 'кг', 20.17, 'Корпус КСО'),
(2, 'material', 'Комплектующие электрические', 100.000, 200.0000, 20000.00, 'компл', 16.81, 'Разъединители, приводы'),
(2, 'material', 'Кабель силовой', 12.500, 1200.0000, 15000.00, 'м', 12.61, 'Внутренние соединения'),
(2, 'material', 'Крепежные изделия', 250.000, 32.0000, 8000.00, 'кг', 6.72, 'Болты, гайки, шайбы'),
(2, 'material', 'Лакокрасочные материалы', 30.000, 450.0000, 13500.00, 'кг', 11.34, 'Покрытие корпуса'),
(2, 'material', 'Изоляторы', 150.000, 30.0000, 4500.00, 'шт', 3.78, 'Опорные изоляторы'),
(2, 'labor', 'Основные рабочие', 600.000, 30.0000, 18000.00, 'час', 15.13, 'Сборка КСО'),
(2, 'labor', 'Сварщики', 150.000, 40.0000, 6000.00, 'час', 5.04, 'Сварочные работы'),
(2, 'overhead', 'Аренда', 1.000, 5000.0000, 5000.00, 'месяц', 4.20, 'Доля аренды'),
(2, 'overhead', 'Амортизация', 1.000, 4000.0000, 4000.00, 'месяц', 3.36, 'Износ оборудования'),
(2, 'overhead', 'Накладные расходы', 1.000, 3000.0000, 3000.00, 'месяц', 2.52, 'Общепроизводственные'),
(2, 'energy', 'Электроэнергия', 3000.000, 0.3500, 1050.00, 'кВт*ч', 0.88, 'Сварка, освещение');
